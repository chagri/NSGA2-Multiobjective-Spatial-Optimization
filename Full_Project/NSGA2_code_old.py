...
class NSGA2MOSO(Benchmark):
	...
	def __init__(self, dimensions=43278):
		...
		gg=open("Refined_Swas_E_Updated2.csv","r") # lt10 minperc maxperc
		datag = gg.readlines()[1:]
		for line in datag:
			line = line.split('\t')
			lt10 = int(line[0])
			

		gh = open("SWAS_E_LU_PERCENT.csv","rw+")
		datagh = gh.readlines()
		for line in datagh:
			line = line.split('\t')
...
#					f[ltidx] = (self.maxA[ltidx] + self.minA[ltidx])/2 - abs(sumIndArea[ltidx] - (self.maxA[ltidx] + self.minA[ltidx])/2 )
		...
		for ltidx in range(1,numTarget+1):
	   		#print ltidx
			if (sumIndArea[ltidx] - self.minA[ltidx] < 0 ):
				f[1] = -(sumIndArea[ltidx] - self.minA[ltidx])
			elif (sumIndArea[ltidx] - self.maxA[ltidx] > 0 ):
				f[1] = (sumIndArea[ltidx] - self.maxA[ltidx])
			else: 
				f[1] += 0
